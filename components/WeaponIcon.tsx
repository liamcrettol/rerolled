...
export default function WeaponIcon({ icon, watermark, name, size = "medium", count }: WeaponIconProps) {
  ...
  return (
    <div className={`relative ${sizeMap[size]} shrink-0 overflow-hidden bg-bungie-dark`}>
      ...
      {count !== undefined && count > 1 && (
        <div className="absolute bottom-0 right-0 bg-bungie-blue text-white text-xs font-bold px-1.5 py-0.5">
          {count}×
        </div>
      )}
    </div>
